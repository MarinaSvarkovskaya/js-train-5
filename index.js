...
function findMinMax([firstElement, ...restElements]) {
  ...
  // let min = firstElement[0];
  // let max = firstElement[0];
  // for (const index of firstElement) {
  //   if (index < min) {
  //     min = index;
  //   }
  //   if (index > man) {
...
function multiplyByIndex(arr) {
  ...
  // Повертаємо масив результату
  let list = [];
  for (const abc in arr) {
    for (const index of arr) {
      list[list.length] = index * abc;
    }
  }
  return list;
}
...
function replaceNumbers(arr) {
  ...
  for (const index of arr) {
    if (index > 10) {
      list[list.length] = "Greater than 10";
    } else if (index <= 10) {
      list[list.length] = "Less than or equal to 10";
    }
  }
  return list;
}
